...
import {
    ...
    updateDeliveryBoy,
    type DeliveryBoy,
    type UpdateDeliveryBoyData,
} from '../../../services/api/admin/adminDeliveryService';
import { useAuth } from '../../../context/AuthContext';
import DeliveryBoyEditModal from '../components/DeliveryBoyEditModal';
...
export default function AdminManageDeliveryBoy() {
    ...
    const handleEditDetails = (deliveryBoy: DeliveryBoy) => {
        setSelectedDeliveryBoy(deliveryBoy);
        setIsEditModalOpen(true);
    };

    const handleSaveEdit = async (id: string, data: UpdateDeliveryBoyData) => {
        try {
            setProcessing(id);
            const response = await updateDeliveryBoy(id, data);

            if (response.success) {
                setSuccessMessage('Delivery boy details updated successfully!');
                ...
                setError('Failed to update delivery boy: ' + (response.message || 'Unknown error'));
                setSuccessMessage('');
            }
        } catch (err: any) {
            console.error('Error updating delivery boy:', err);
            setError('Failed to update delivery boy: ' + (err.response?.data?.message || 'Please try again.'));
            setSuccessMessage('');
        } finally {
            setProcessing(null);
        }
                                        ...
                                        <tr key={deliveryBoy._id} className="hover:bg-neutral-50 transition-colors text-sm text-neutral-700 border-b border-neutral-200">
                                            ...
                                            <td className="p-4 align-middle">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => handleEditDetails(deliveryBoy)}

...
export default function ProductDetail() {
  ...
  const [activeShareId, setActiveShareId] = useState<boolean>(false);
  ...
  const isVariantAvailable =
    selectedVariant?.status !== "Sold out"; // 0 means unlimited, so we only check status
  ...
  const handleShare = async () => {
    ...
    setActiveShareId(!activeShareId);
  };
        ...
        <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-neutral-100 px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              {activeShareId && (
                <div 
                  className="absolute right-0 top-11 w-44 bg-white rounded-xl shadow-2xl border border-neutral-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200"
                  onClick={(e) => e.stopPropagation()}
                >
                  <button 
                    onClick={handleWhatsAppShare}
                    className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-neutral-700 hover:bg-green-50 hover:text-green-600 transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-neutral-700 hover:bg-neutral-50 transition-colors"
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-neutral-400"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                    Copy Link
                  </button>
                </div>
              )}
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="hidden md:flex items-center gap-3">
                    <div className="relative">
                      <button 
                        onClick={handleShare}
                        className="w-10 h-10 rounded-full border border-neutral-200 flex items-center justify-center text-neutral-500 hover:text-blue-600 hover:border-blue-100 hover:bg-blue-50 transition-all shadow-sm"
                        title="Share product"
                      >
                      ...
                      {activeShareId && (
                        <div 
                          className="absolute right-0 top-12 w-44 bg-white rounded-xl shadow-2xl border border-neutral-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button 
                            onClick={handleWhatsAppShare}
                    ...
                    {product?.id && (
                      <WishlistButton
                        productId={product.id}
                        size="md"
                        className="relative top-0 right-0 shadow-sm border border-neutral-200"
                      />
                    )}
                  </div>
                </div>

...
export default function OrderDetail() {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `Track my KlydoCart order #${orderNumber}: ${orderUrl}`,
      url: orderUrl,
    };
    ...
    if (navigator.share) {
      try {
        ...
        if ((err as Error).name !== 'AbortError') {
          showToast('Failed to share order tracking', 'error');
        }
        return;
      }
    }
  ...
  const handleWhatsAppShare = () => {
    ...
    const appBaseUrl = import.meta.env.VITE_APP_URL || window.location.origin;
    const orderUrl = `${appBaseUrl}/orders/${orderId}`;
    const text = `Track my KlydoCart order #${orderNumber}: ${orderUrl}`;
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <Button 
                  variant="outline" 
                  onClick={handleShare}
                  className="rounded-none px-6 border-gray-200 hover:bg-gray-50 transition-colors">
                ...
                {isShareMenuOpen && (
                  <div 
                    className="absolute right-0 top-12 w-48 bg-white rounded-xl shadow-2xl border border-neutral-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <button 
                      onClick={handleWhatsAppShare}
